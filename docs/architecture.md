...
状态：Milestone 1 已实现；Milestone 2 的 M2.1–M2.4 已完成、M2.5 按用户要求延期；Milestone 3、Milestone 4 已通过自动化与 H5 交互验证；Milestone 5 本机备份与“全部覆盖 / 本机优先合并”双模式恢复的 M5.1–M5.5 已实现，并完成写锁范围、失败媒体回收、启动期孤儿目录清理和文件选择监听解绑闭环；M5.6 Android 自定义基座与真机验收待授权
目标平台：Android App  
...
Android 本地备份与恢复不在 Milestone 2 开发，整体调整到 Milestone 5。2026-07-23 已完成仅本机数据备份，以及“全部覆盖 / 本机优先合并”双模式恢复的 v1 设计与 M5.1–M5.5 运行时实现；备份一致性快照完成后会先释放写锁再进入系统文件选择器，恢复失败会回收本次媒体 generation，启动时会清理超过 24 小时且无数据库引用的恢复/临时目录。Android 真机验收仍延期。
...
v1 不包含逐记录选择、逐字段冲突编辑、定时备份、WebDAV/iCloud、分享、加密或跨设备同步。当前运行时入口只在 Android App 注册；H5 不显示入口，也不提供模拟备份实现。
...
2026-07-23 用户已授权并完成本机数据备份与恢复 v1 的 M5.1–M5.5 实现；范围不扩展到 WebDAV、iCloud、分享、通知、系统日历、OCR、地图、桌面组件、购买或发布能力，M5.6 自定义基座与真机验收仍待单独授权。
...
5. **M5.5 双模式恢复（已完成代码、自动化与资源构建）**：文件检查、模式与计划预览、内部恢复点、媒体 generation、SQLite 覆盖/合并事务和撤销；备份生成结束后先释放写锁再进入 SAF，恢复写入失败立即回收本次 generation，提交后临时清理失败不误报事务失败，启动时保留数据库仍引用的 generation 并清理超过 24 小时的无引用恢复/工作目录，备份页卸载会解除待处理的文件选择监听；
