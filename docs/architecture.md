...
- 快速添加“韩国音乐剧排期”参考 iOS `SelectYyjKorea.swift`，仅通过 `https://myukit.com/today?date=yyyy-MM-dd` 按日期查询并按开演时间分组；选择场次后可分别勾选名称、时间、剧目和阵容，未勾选字段不得覆盖当前草稿。该数据源不提供城市和场地，Android 不臆造地点、不接入地图坐标；H5 仅通过开发代理绕过跨域，App 直接请求 HTTPS。设置页“应用设置 > 快速添加”可分别控制复制、中文音乐剧排期和韩国音乐剧排期，偏好与中文排期最近查询城市保存在 Android `app_settings`；解析链接、识图和批量导入尚未实现，也不得显示为可用选项；
...
3. **M2.3 演出编辑闭环（实现及 H5 交互门已通过，持续进行功能还原校准）**：新增/编辑表单、字段校验、相近两小时提醒、海报和票根事务边界已实现；城市与场地改为独立选择流程，包含默认地区、其它地区、城市搜索、常去城市和按城市筛选场地，不移植 iOS 地图选择；快速添加已实现复制已有演出、中文音乐剧排期、韩国音乐剧排期及各自字段勾选，开关由“应用设置 > 快速添加”持久化管理；Android Photo Picker、复制海报独立落盘、图片压缩、沙盒文件及排期网络请求真机验证并入 M2.5；
