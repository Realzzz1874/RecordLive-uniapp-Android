...
- 海报、票根；
- 阵容、嘉宾、剧目、渠道、朋友、出品方；
- 阵容录入参考 iOS 的选择页交互：表单只显示“添加阵容”或已选名称，进入独立页面后逐个手动添加，并支持修改、删除和调整顺序；Android 内置从 iOS 原样复制的 `src/features/performances/names.json`，输入关键字时将该预置名单与已有记录阵容合并为预选结果；不实现识图与批量输入模式；
- 票面价、实付价、其他花费及币种；
- 座位、评分。
...
4. **M3.4 待看闭环（H5 交互门已通过）**：待看 Tab 展示未来正常演出和待开票演出，复用详情、编辑、删除和新增流程；
